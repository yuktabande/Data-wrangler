  ...
  return async (env) => {
    ...
    if (typeof configOrOptions === "function") {
      ...
      if ("cloudflare" in optionObject) {
        if (optionObject.cloudflare === false && !("nitro" in optionObject)) {
          optionObject.nitro = false;
          logWarning(
            "`cloudflare: false` is deprecated in v2 \u2014 migrated to `nitro: false`. Update your config to drop the warning."
          );
        } else {
          logWarning(
    ...
    const shouldRunNitro = options.nitro !== false && command === "build" && (explicitNitro || isSandbox);
    if (options.nitro === void 0 && command === "build" && !isSandbox) {
      console.info(
    ...
    if (shouldRunNitro) {
      ...
      try {
        ...
        if (explicitNitro || isSandbox) {
          ...
          throw new Error(
            `[@lovable.dev/vite-tanstack-config] ${reason}, but the \`nitro\` package isn't installed. Add \`nitro\` to your devDependencies (matches peerDependency >=3.0.260429-beta). Original import error: ${err instanceof Error ? err.message : String(err)}`
          );
        }
      }
      if (nitroMod) {
        ...
        const nitroOpts = { ...userNitroOpts, preset, output };
        if (preset === "cloudflare-module") {
          nitroOpts.cloudflare = { nodeCompat: true, deployConfig: true, ...userNitroOpts.cloudflare };
        }
